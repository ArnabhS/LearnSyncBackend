...
const submitAnswers = async (req, res) => {
  ...
  const combinedQuestionAnswerPairs = [...previousQuestionAnswerPairs, ...newQuestionAnswerPairs];

  // Update the number of questions answered
  user.questionsAnswered += answers.length;

  // Analyze disorders based on combined answers
  const { adhdScore, autismScore, dyslexiaScore } = await analyzeDisorders(combinedQuestionAnswerPairs);
